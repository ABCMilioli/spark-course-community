...
// Adicionar log para todas as requisiï¿½ï¿½es
...
// Configuraï¿½ï¿½o do Multer para upload de arquivos
const upload = multer({
...
// Configuraï¿½ï¿½o do MinIO (usando variï¿½veis de ambiente da stack)
...
// Validaï¿½ï¿½o das variï¿½veis de ambiente do MinIO
...
if (missingVars.length > 0) {
  console.warn('[MinIO] Variï¿½veis de ambiente ausentes:', missingVars);
  console.warn('[MinIO] Upload de arquivos serï¿½ desabilitado');
} else {
  console.log('[MinIO] Configuraï¿½ï¿½o carregada com sucesso');
...
const authenticateToken = (req, res, next) => {
  process.stdout.write(`\n[AUTH] === INï¿½CIO DA AUTENTICAï¿½ï¿½O ===\n`);
  ...
  if (!auth) {
    ...
    process.stdout.write('[AUTH] === FIM DA AUTENTICAï¿½ï¿½O (erro) ===\n\n');
  ...
  try {
    ...
    process.stdout.write(`[AUTH] Token vï¿½lido para usuï¿½rio: ${JSON.stringify(decoded)}\n`);
    process.stdout.write('[AUTH] === FIM DA AUTENTICAï¿½ï¿½O (sucesso) ===\n\n');
    req.user = decoded;
    next();
  } catch (err) {
    process.stdout.write(`[AUTH] Token invï¿½lido: ${err.message}\n`);
    process.stdout.write(`[AUTH] Stack trace: ${err.stack}\n`);
    process.stdout.write('[AUTH] === FIM DA AUTENTICAï¿½ï¿½O (erro) ===\n\n');
    res.status(401).json({ error: 'Token invï¿½lido.' });
  }
};
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (!email || !password) return res.status(400).json({ error: 'Email e senha obrigatï¿½rios.' });

  try {
    const { rows } = await pool.query('SELECT * FROM profiles WHERE email = $1', [email]);
    const user = rows[0];
    if (!user) return res.status(401).json({ error: 'Usuï¿½rio nï¿½o encontrado.' });

    // Se nï¿½o existir campo de senha, aceite qualquer senha para o admin inicial
    ...
    if (!valid) return res.status(401).json({ error: 'Senha invï¿½lida.' });
...
// Endpoint para perfil pï¿½blico de usuï¿½rio
app.get('/api/users/:userId/profile', authenticateToken, async (req, res) => {
  try {
    const { userId } = req.params;
    console.log('[GET /api/users/:userId/profile] Buscando perfil pï¿½blico para usuï¿½rio:', userId);
    
    // Buscar dados bï¿½sicos do usuï¿½rio (apenas campos pï¿½blicos)
    ...
    if (userResult.rows.length === 0) {
      return res.status(404).json({ error: 'Usuï¿½rio nï¿½o encontrado.' });
    }
    
    const user = userResult.rows[0];
    
    // Buscar estatï¿½sticas do usuï¿½rio
    ...
    // Buscar posts recentes do usuï¿½rio (ï¿½ltimos 5)
    ...
    console.log('[GET /api/users/:userId/profile] Perfil pï¿½blico encontrado:', {
      id: user.id,
      name: user.name,
      stats
    });
    ...
    console.error('[GET /api/users/:userId/profile] Erro ao buscar perfil pï¿½blico:', err);
    res.status(500).json({ error: 'Erro interno.' });
  }
});

// ===== UPLOAD DE IMAGENS DO Fï¿½RUM =====
app.post('/api/forum/upload-image', authenticateToken, upload.single('image'), async (req, res) => {
  try {
    ...
    if (!allowedTypes.includes(req.file.mimetype)) {
      return res.status(400).json({ error: 'Tipo de arquivo nï¿½o permitido. Use JPEG, PNG, GIF ou WebP.' });
    }

    // Validar tamanho (mï¿½ximo 10MB)
    const maxSize = 10 * 1024 * 1024; // 10MB
    if (req.file.size > maxSize) {
      return res.status(400).json({ error: 'Arquivo muito grande. Tamanho mï¿½ximo: 10MB.' });
    }

    // Fazer upload para MinIO
    const result = await uploadFile(req.file, 'forum-images');
    
    console.log('[POST /api/forum/upload-image] Upload concluï¿½do:', result.url);
...
// ===== SISTEMA DE Fï¿½RUM: GET Tï¿½PICOS =====
app.get('/api/forum/topics', authenticateToken, async (req, res) => {
  process.stdout.write('\n[DEBUG] ========== Inï¿½cio GET /api/forum/topics ==========\n');
  process.stdout.write(`[DEBUG] Usuï¿½rio: ${JSON.stringify(req.user)}\n`);
  try {
    process.stdout.write('[GET /api/forum/topics] Iniciando busca de tï¿½picos do fï¿½rum...\n');
    ...
    process.stdout.write(`[GET /api/forum/topics] Tï¿½picos encontrados: ${rows.length}\n`);
    process.stdout.write(`[GET /api/forum/topics] IDs dos tï¿½picos: ${rows.map(r => r.id).join(', ')}\n`);
    
    if (!Array.isArray(rows)) {
      process.stdout.write('[GET /api/forum/topics] Erro: rows nï¿½o ï¿½ um array\n');
      res.json([]);
      return;
    }
    ...
    process.stdout.write(`[GET /api/forum/topics] Erro ao buscar tï¿½picos: ${err}\n`);
    process.stdout.write(`[GET /api/forum/topics] Stack trace: ${err.stack}\n`);
    res.status(500).json({ error: 'Erro interno ao buscar tï¿½picos.' });
...
app.put('/api/profile', authenticateToken, async (req, res) => {
  try {
    ...
    if (!name || !email) {
      return res.status(400).json({ error: 'Nome e email sï¿½o obrigatï¿½rios.' });
    }
    
    // Verificar se o email jï¿½ existe (exceto para o usuï¿½rio atual)
    ...
    if (emailCheck.rows.length > 0) {
      return res.status(409).json({ error: 'Este email jï¿½ estï¿½ em uso.' });
    }
...
// Endpoint de cadastro de usuï¿½rio
app.post('/api/auth/signup', async (req, res) => {
  const { name, email, password } = req.body;
  if (!name || !email || !password) return res.status(400).json({ error: 'Nome, email e senha obrigatï¿½rios.' });
  try {
    const { rows } = await pool.query('SELECT * FROM profiles WHERE email = $1', [email]);
    if (rows.length > 0) return res.status(409).json({ error: 'Email jï¿½ cadastrado.' });
    const hash = await bcrypt.hash(password, 10);
    const id = crypto.randomUUID();
    await pool.query('INSERT INTO profiles (id, name, email, password_hash, role) VALUES ($1, $2, $3, $4, $5)', [id, name, email, hash, 'student']);
    res.status(201).json({ message: 'Usuï¿½rio criado com sucesso.' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Erro interno.' });
  }
...
app.post('/api/posts', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[POST /api/posts] Usuï¿½rio autenticado:', req.user);

    if (!title || !content) {
      console.log('[POST /api/posts] Falha: tï¿½tulo ou conteï¿½do ausente');
      return res.status(400).json({ error: 'Tï¿½tulo e conteï¿½do sï¿½o obrigatï¿½rios.' });
    }
    const id = crypto.randomUUID();
    ...
    // Criar notificaï¿½ï¿½o para moderadores/admins sobre novo post na comunidade
    try {
      ...
      console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de novo post da comunidade:', notificationErr);
    }
...
app.get('/api/courses', authenticateToken, async (req, res) => {
  try {
    ...
    // Para cada curso, buscar mï¿½dulos e aulas e calcular total_duration
...
// Endpoint para detalhes de um curso especï¿½fico
app.get('/api/courses/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    
    // Buscar dados bï¿½sicos do curso
    const { rows } = await pool.query(`
    ...
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Curso nï¿½o encontrado.' });
    }
    
    const course = rows[0];
    console.log('[GET /api/courses/:id] Dados do curso:', course);
    
    // Buscar mï¿½dulos e aulas
    let modules = [];
    try {
      ...
      `, [id]);
      // Estruturar os dados dos mï¿½dulos
      modules = [];
      ...
      console.error('[GET /api/courses/:id] Erro ao calcular mï¿½dulos/duraï¿½ï¿½o:', err);
      return res.status(500).json({ error: 'Erro ao calcular mï¿½dulos/duraï¿½ï¿½o.' });
    }
...
app.get('/api/courses/:id/admin', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`[GET /api/courses/${id}/admin] Requisiï¿½ï¿½o recebida`);
    console.log(`[GET /api/courses/${id}/admin] Usuï¿½rio:`, req.user);
    
    // Verificar se o usuï¿½rio ï¿½ admin ou instructor
    ...
    if (courseResult.rows.length === 0) {
      console.log(`[GET /api/courses/${id}/admin] Curso nï¿½o encontrado`);
      return res.status(404).json({ error: 'Curso nï¿½o encontrado.' });
    }
    ...
    console.log(`[GET /api/courses/${id}/admin] Mï¿½dulos encontrados:`, modulesResult.rows.length);
    ...
    console.log(`[GET /api/courses/${id}/admin] Resposta enviada com ${modules.length} mï¿½dulos`);
    res.json(response);
  } catch (err) {
...
// Endpoint para salvar dados administrativos do curso (incluindo mï¿½dulos e aulas)
    ...
    try {
      ...
      if (modules && modules.length > 0) {
        for (let i = 0; i < modules.length; i++) {
          ...
          // Inserir aulas do mï¿½dulo
          if (module.lessons && module.lessons.length > 0) {
            for (let j = 0; j < module.lessons.length; j++) {
              const lesson = module.lessons[j];
              const lessonId = crypto.randomUUID();
              
              // Log para depuraï¿½ï¿½o do conteï¿½do da aula
...
// Endpoint para dados do player de vï¿½deo
app.get('/api/courses/:id/player', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    
    // Verificar se o usuï¿½rio estï¿½ matriculado no curso
    ...
    if (enrollmentCheck.rows.length === 0 && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Vocï¿½ precisa estar matriculado neste curso.' });
    }
    ...
    rows.forEach(row => {
      ...
      // Sï¿½ incluir aulas visï¿½veis
      if (row.lesson_id && row.lesson_is_visible && currentModule) {
        // Verificar se a aula deve ser liberada baseado na data de matrï¿½cula
    ...
    // Funï¿½ï¿½o auxiliar para buscar aulas concluï¿½das
...
// Endpoint para buscar matrï¿½culas do usuï¿½rio
app.get('/api/enrollments', authenticateToken, async (req, res) => {
  try {
    const { user_id, course_id } = req.query;
    
    if (!user_id && !course_id) {
      return res.status(400).json({ error: 'user_id ou course_id ï¿½ obrigatï¿½rio.' });
    }
    
    // Buscar as matrï¿½culas e dados bï¿½sicos do curso
    let query = `
      SELECT e.*, 
    ...
    // Para cada curso, buscar mï¿½dulos e aulas e calcular total_lessons, total_duration e progresso
    const results = await Promise.all(rows.map(async (enrollment) => {
      // Buscar mï¿½dulos e aulas do curso
      const modulesQuery = `
      ...
      // Montar estrutura de mï¿½dulos e aulas
      const modulesMap = {};
      const allLessonIds = [];
      moduleRows.forEach(row => {
        if (!row.module_id) return;
      ...
      const modules = Object.values(modulesMap);
      // Calcular total de aulas e duraï¿½ï¿½o
      ...
      // Buscar aulas concluï¿½das pelo usuï¿½rio
    ...
    console.error('[GET /api/enrollments] Erro ao buscar matrï¿½culas:', err);
    res.status(500).json({ error: 'Erro interno.' });
  }
});
...
app.get('/api/popular-tags', authenticateToken, async (req, res) => {
  try {
    ...
    // Se nï¿½o houver tags suficientes, adicionar algumas padrï¿½o
...
// Servir arquivos estï¿½ticos do React
...
app.post('/api/courses', authenticateToken, async (req, res) => {
  try {
    ...
    if (!title || !description || !level) {
      return res.status(400).json({ error: 'Campos obrigatï¿½rios nï¿½o preenchidos.' });
    }

    // Validaï¿½ï¿½o do preï¿½o baseada no tipo de curso
    let finalPrice = 0;
    if (isPaid) {
      if (!price || parseFloat(price) <= 0) {
        return res.status(400).json({ error: 'Para cursos pagos, o preï¿½o deve ser maior que zero.' });
      }
...
// Buscar post especï¿½fico
app.get('/api/posts/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    console.log('[GET /api/posts/:id] Buscando post:', id);
    console.log('[GET /api/posts/:id] Usuï¿½rio autenticado:', req.user);
    ...
    if (rows.length === 0) {
      console.log('[GET /api/posts/:id] Post nï¿½o encontrado:', id);
      return res.status(404).json({ error: 'Post nï¿½o encontrado.' });
    }
...
app.put('/api/posts/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'Post nï¿½o encontrado ou sem permissï¿½o.' });
    }
...
app.put('/api/courses/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'Curso nï¿½o encontrado ou sem permissï¿½o.' });
    }
    res.json(result.rows[0]);
  } catch (err) {
...
app.put('/api/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'Usuï¿½rio nï¿½o encontrado ou sem permissï¿½o.' });
    }
    res.json(result.rows[0]);
  } catch (err) {
    console.error('[PUT /api/users/:id] Erro ao editar usuï¿½rio:', err);
    res.status(500).json({ error: 'Erro ao editar usuï¿½rio.' });
  }
});
...
app.delete('/api/users/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ message: 'Usuï¿½rio deletado com sucesso.' });
  } catch (err) {
    console.error('[DELETE /api/users/:id] Erro ao deletar usuï¿½rio:', err);
    res.status(500).json({ error: 'Erro ao deletar usuï¿½rio.' });
  }
});
...
app.get('/api/test-data', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[GET /api/test-data] Usuï¿½rios encontrados:', usersResult.rows);
    ...
    // Verificar matrï¿½culas
    const enrollmentsResult = await pool.query('SELECT id, user_id, course_id FROM enrollments LIMIT 5');
    console.log('[GET /api/test-data] Matrï¿½culas encontradas:', enrollmentsResult.rows);
  ...
  try {
    ...
    if (parseInt(postsCheck.rows[0].count) === 0) {
      ...
      await pool.query(
        ...
        [post1Id, 'Meu primeiro post', 'Este ï¿½ o conteï¿½do do meu primeiro post na comunidade!', req.user.id, 'geral', new Date()]
      );
      
      await pool.query(
        'INSERT INTO posts (id, title, content, author_id, category, created_at) VALUES ($1, $2, $3, $4, $5, $6)',
        [post2Id, 'Dï¿½vida sobre React', 'Alguï¿½m pode me ajudar com hooks no React?', req.user.id, 'programaï¿½ï¿½o', new Date()]
      );
      
      console.log('[POST /api/test-data] Posts criados com sucesso');
    }
    
    // Verificar se jï¿½ existem cursos
    ...
    if (parseInt(coursesCheck.rows[0].count) === 0) {
      ...
      await pool.query(
        ...
        [courseId, 'Curso de React Bï¿½sico', 'Aprenda os fundamentos do React', 'Iniciante', 99.99, req.user.id, new Date()]
      );
      
      console.log('[POST /api/test-data] Curso criado com sucesso');
    }
    
    // Verificar se jï¿½ existem matrï¿½culas
    ...
    if (parseInt(enrollmentsCheck.rows[0].count) === 0) {
      console.log('[POST /api/test-data] Criando matrï¿½culas de teste...');
      ...
      if (courseResult.rows.length > 0) {
        ...
        console.log('[POST /api/test-data] Matrï¿½cula criada com sucesso');
      }
    }
...
app.get('/api/posts-check', authenticateToken, async (req, res) => {
  try {
    ...
    if (postsCount === 0) {
      ...
      await pool.query(
        'INSERT INTO posts (id, title, content, author_id, category, created_at) VALUES ($1, $2, $3, $4, $5, $6)',
        [postId, 'Post de Teste', 'Este ï¿½ um post de teste criado automaticamente.', req.user.id, 'teste', new Date()]
      );
...
app.post('/api/posts/:id/like', authenticateToken, async (req, res) => {
  try {
    ...
    // Verificar se jï¿½ curtiu (para notificar apenas na primeira curtida)
    const existingLike = await pool.query(
      'SELECT 1 FROM post_likes WHERE post_id = $1 AND user_id = $2',
      [postId, userId]
    );
    
    // Tenta inserir, ignora se jï¿½ existe
    const result = await pool.query(
      'INSERT INTO post_likes (post_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING RETURNING *',
      [postId, userId]
    );
    
    // Se foi uma nova curtida (nï¿½o existia antes), criar notificaï¿½ï¿½o
    if (existingLike.rows.length === 0 && result.rows.length > 0) {
      try {
        ...
        console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de curtida em post da comunidade:', notificationErr);
      }
    }
...
// Listar comentï¿½rios de um post
app.get('/api/posts/:id/comments', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar comentï¿½rios.' });
  }
});

// Adicionar comentï¿½rio a um post
app.post('/api/posts/:id/comments', authenticateToken, async (req, res) => {
  try {
    ...
    if (!content || content.trim().length === 0) {
      return res.status(400).json({ error: 'Comentï¿½rio nï¿½o pode ser vazio.' });
    }
    const result = await pool.query(
    ...
    // Criar notificaï¿½ï¿½o para o autor do post sobre novo comentï¿½rio
    try {
      const postAuthorResult = await pool.query(`
        SELECT p.author_id, p.title, prof.name as author_name
        FROM posts p
      ...
      console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de comentï¿½rio em post da comunidade:', notificationErr);
    }

    res.status(201).json(result.rows[0]);
  } catch (err) {
    console.error('[POST /api/posts/:id/comments] Erro:', err);
    res.status(500).json({ error: 'Erro ao adicionar comentï¿½rio.' });
  }
});
...
app.put('/api/comments/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'Comentï¿½rio nï¿½o encontrado ou sem permissï¿½o.' });
    }
    res.json(result.rows[0]);
  } catch (err) {
    console.error('[PUT /api/comments/:id] Erro:', err);
    res.status(500).json({ error: 'Erro ao editar comentï¿½rio.' });
  }
});
...
app.delete('/api/comments/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ message: 'Comentï¿½rio deletado com sucesso.' });
  } catch (err) {
    console.error('[DELETE /api/comments/:id] Erro:', err);
    res.status(500).json({ error: 'Erro ao deletar comentï¿½rio.' });
  }
});
...
// Verificar se o usuï¿½rio favoritou
...
app.post('/api/classes', authenticateToken, async (req, res) => {
  try {
    console.log('[POST /api/classes] Requisiï¿½ï¿½o recebida');
    console.log('[POST /api/classes] Usuï¿½rio autenticado:', req.user);
    ...
    if (!course_id || !instance_name) {
      console.log('[POST /api/classes] Erro: course_id e instance_name sï¿½o obrigatï¿½rios');
      return res.status(400).json({ error: 'ID do curso e nome da turma sï¿½o obrigatï¿½rios.' });
    }
    
    // Verificar se o usuï¿½rio ï¿½ instructor ou admin
    if (!['instructor', 'admin'].includes(req.user.role)) {
      console.log('[POST /api/classes] Erro: usuï¿½rio nï¿½o tem permissï¿½o, role:', req.user.role);
...
app.get('/api/classes/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (classResult.rows.length === 0) {
      console.log('[GET /api/classes/:id] Turma nï¿½o encontrada');
      return res.status(404).json({ error: 'Turma nï¿½o encontrada.' });
    }
    
    const classData = classResult.rows[0];
    console.log('[GET /api/classes/:id] Turma encontrada:', classData.instance_name);
    
    // Verificar se o usuï¿½rio tem acesso ï¿½ turma
    // 1. Se ï¿½ o instructor da turma
    if (classData.instructor_id === req.user.id) {
      console.log('[GET /api/classes/:id] Usuï¿½rio ï¿½ o instructor da turma');
      return res.json(classData);
    }
    
    // 2. Se a turma ï¿½ pï¿½blica
    if (classData.is_public) {
      console.log('[GET /api/classes/:id] Turma ï¿½ pï¿½blica, acesso permitido');
      return res.json(classData);
    }
    
    // 3. Se o usuï¿½rio estï¿½ matriculado na turma
    const enrollmentCheck = await pool.query(`
    ...
    if (enrollmentCheck.rows.length > 0) {
      console.log('[GET /api/classes/:id] Usuï¿½rio estï¿½ matriculado na turma');
      return res.json(classData);
    }
    
    console.log('[GET /api/classes/:id] Acesso negado - usuï¿½rio nï¿½o tem permissï¿½o');
...
app.post('/api/classes/:id/enroll', authenticateToken, async (req, res) => {
  try {
    ...
    if (classInfo.rows[0].max_students && 
      ...
      return res.status(400).json({ error: 'Turma estï¿½ lotada.' });
    }
    ...
    res.status(201).json({ message: 'Usuï¿½rio matriculado com sucesso.' });
  } catch (err) {
    console.error('[POST /api/classes/:id/enroll] Erro:', err);
    res.status(500).json({ error: 'Erro ao matricular usuï¿½rio.' });
  }
});

// Listar matrï¿½culas de uma turma
app.get('/api/classes/:id/enrollments', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    console.log('[GET /api/classes/:id/enrollments] Buscando matrï¿½culas da turma:', id);
    console.log('[GET /api/classes/:id/enrollments] Usuï¿½rio:', req.user);
    
    // Primeiro, buscar a turma para verificar se o usuï¿½rio ï¿½ o instructor
    const classResult = await pool.query(`
      SELECT instructor_id, is_public FROM class_instances WHERE id = $1
    `, [id]);
    
    if (classResult.rows.length === 0) {
      console.log('[GET /api/classes/:id/enrollments] Turma nï¿½o encontrada');
      return res.status(404).json({ error: 'Turma nï¿½o encontrada.' });
    }
    
    const classData = classResult.rows[0];
    
    // Verificar se o usuï¿½rio tem acesso ï¿½s matrï¿½culas
    // 1. Se ï¿½ o instructor da turma
    if (classData.instructor_id === req.user.id) {
      console.log('[GET /api/classes/:id/enrollments] Usuï¿½rio ï¿½ o instructor da turma');
      const { rows } = await pool.query(`
        SELECT 
          cie.*,
          p.name as user_name,
    ...
    // 2. Se a turma ï¿½ pï¿½blica, permitir visualizar matrï¿½culas
    if (classData.is_public) {
      console.log('[GET /api/classes/:id/enrollments] Turma ï¿½ pï¿½blica, acesso permitido');
      const { rows } = await pool.query(`
        SELECT 
          cie.*,
          p.name as user_name,
    ...
    if (enrollmentCheck.rows.length > 0) {
      console.log('[GET /api/classes/:id/enrollments] Usuï¿½rio estï¿½ matriculado na turma');
      const { rows } = await pool.query(`
        SELECT 
          cie.*,
          p.name as user_name,
    ...
    console.log('[GET /api/classes/:id/enrollments] Acesso negado - usuï¿½rio nï¿½o tem permissï¿½o');
    return res.status(403).json({ error: 'Acesso negado a esta turma.' });
...
app.post('/api/classes/:id/content', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    ...
    if (!title || (!content && !req.file)) {
      return res.status(400).json({ error: 'Tï¿½tulo e conteï¿½do ou arquivo sï¿½o obrigatï¿½rios.' });
    }
    
    // Verificar se a turma existe e se o usuï¿½rio tem permissï¿½o
    const classCheck = await pool.query(`
      SELECT instructor_id FROM class_instances WHERE id = $1
    `, [id]);
    ...
    // Verificar se o usuï¿½rio ï¿½ o instructor da turma ou admin
    if (classCheck.rows[0].instructor_id !== req.user.id && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Apenas o instructor da turma pode criar conteï¿½do.' });
    }
    
    let fileData = null;
    
    // Se hï¿½ um arquivo, fazer upload para o MinIO
    if (req.file) {
      try {
    ...
    res.status(500).json({ error: 'Erro ao criar conteï¿½do.' });
  }
});

// Listar conteï¿½do de uma turma
app.get('/api/classes/:id/content', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    console.log('[GET /api/classes/:id/content] Buscando conteï¿½do da turma:', id);
    console.log('[GET /api/classes/:id/content] Usuï¿½rio:', req.user);
    ...
    if (classResult.rows.length === 0) {
      console.log('[GET /api/classes/:id/content] Turma nï¿½o encontrada');
      return res.status(404).json({ error: 'Turma nï¿½o encontrada.' });
    }
    
    const classData = classResult.rows[0];
    
    // Verificar se o usuï¿½rio tem acesso ao conteï¿½do
    // 1. Se ï¿½ o instructor da turma
    if (classData.instructor_id === req.user.id) {
      console.log('[GET /api/classes/:id/content] Usuï¿½rio ï¿½ o instructor da turma');
      const { rows } = await pool.query(`
        SELECT 
          cic.*,
    ...
    // 2. Se a turma ï¿½ pï¿½blica, permitir visualizar conteï¿½do
    if (classData.is_public) {
      console.log('[GET /api/classes/:id/content] Turma ï¿½ pï¿½blica, acesso permitido');
      const { rows } = await pool.query(`
        SELECT 
          cic.*,
          p.name as author_name,
    ...
    if (enrollmentCheck.rows.length > 0) {
      console.log('[GET /api/classes/:id/content] Usuï¿½rio estï¿½ matriculado na turma');
      const { rows } = await pool.query(`
        SELECT 
          cic.*,
          p.name as author_name,
    ...
    console.log('[GET /api/classes/:id/content] Acesso negado - usuï¿½rio nï¿½o tem permissï¿½o');
    return res.status(403).json({ error: 'Acesso negado a esta turma.' });
...
app.put('/api/classes/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[PUT /api/classes/:id] Usuï¿½rio:', req.user);
    ...
    if (classCheck.rows.length === 0) {
      console.log('[PUT /api/classes/:id] Turma nï¿½o encontrada');
      return res.status(404).json({ error: 'Turma nï¿½o encontrada.' });
    }
    
    // Verificar se o usuï¿½rio ï¿½ o instructor da turma ou admin
    if (classCheck.rows[0].instructor_id !== req.user.id && req.user.role !== 'admin') {
      console.log('[PUT /api/classes/:id] Acesso negado - usuï¿½rio nï¿½o ï¿½ instructor da turma');
      return res.status(403).json({ error: 'Apenas o instructor da turma pode editï¿½-la.' });
    }
    
    // Validar dados obrigatï¿½rios
    if (!instance_name || instance_name.trim().length === 0) {
      console.log('[PUT /api/classes/:id] Erro: nome da turma nï¿½o fornecido');
      return res.status(400).json({ error: 'Nome da turma ï¿½ obrigatï¿½rio.' });
    }
...
app.delete('/api/classes/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[DELETE /api/classes/:id] Usuï¿½rio:', req.user);
    ...
    if (classCheck.rows.length === 0) {
      console.log('[DELETE /api/classes/:id] Turma nï¿½o encontrada');
      return res.status(404).json({ error: 'Turma nï¿½o encontrada.' });
    }
    
    // Verificar se o usuï¿½rio ï¿½ o instructor da turma ou admin
    if (classCheck.rows[0].instructor_id !== req.user.id && req.user.role !== 'admin') {
      console.log('[DELETE /api/classes/:id] Acesso negado - usuï¿½rio nï¿½o ï¿½ instructor da turma');
      return res.status(403).json({ error: 'Apenas o instructor da turma pode excluï¿½-la.' });
    }
    
    const className = classCheck.rows[0].instance_name;
    
    // Excluir turma (as tabelas relacionadas serï¿½o excluï¿½das automaticamente devido ao CASCADE)
    await pool.query(`
      DELETE FROM class_instances WHERE id = $1
    `, [id]);
    
    console.log('[DELETE /api/classes/:id] Turma excluï¿½da com sucesso:', className);
    res.json({ message: 'Turma excluï¿½da com sucesso.' });
...
// Endpoint para criar matrï¿½cula em curso
app.post('/api/enrollments', authenticateToken, async (req, res) => {
  try {
    const { course_id } = req.body;
    
    console.log('[POST /api/enrollments] Criando matrï¿½cula');
    console.log('[POST /api/enrollments] course_id:', course_id);
    console.log('[POST /api/enrollments] req.user:', req.user);
    
    if (!course_id) {
      return res.status(400).json({ error: 'course_id ï¿½ obrigatï¿½rio.' });
    }

    // Verifica se jï¿½ estï¿½ matriculado
    const existing = await pool.query(
      'SELECT id FROM enrollments WHERE user_id = $1 AND course_id = $2',
      [req.user.id, course_id]
    );
    
    console.log('[POST /api/enrollments] Verificaï¿½ï¿½o de matrï¿½cula existente:', existing.rows);
    
    if (existing.rows.length > 0) {
      console.log('[POST /api/enrollments] Usuï¿½rio jï¿½ estï¿½ matriculado');
      return res.status(400).json({ error: 'Usuï¿½rio jï¿½ estï¿½ matriculado.' });
    }

    const id = crypto.randomUUID();
    console.log('[POST /api/enrollments] Criando matrï¿½cula com ID:', id);
    ...
    // --- NOVA Lï¿½GICA: Matricular automaticamente em todas as turmas do curso ---
    ...
    for (const turma of classInstances.rows) {
      // Verifica se jï¿½ estï¿½ matriculado na turma
    ...
    console.log('[POST /api/enrollments] Matrï¿½cula criada com sucesso');
    res.status(201).json({ message: 'Matrï¿½cula realizada com sucesso.' });
  } catch (err) {
    console.error('[POST /api/enrollments] Erro:', err);
    res.status(500).json({ error: 'Erro ao realizar matrï¿½cula.' });
  }
});
...
app.post('/api/upload/thumbnail', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    ...
    // Verificar se ï¿½ uma imagem
    if (!req.file.mimetype.startsWith('image/')) {
      return res.status(400).json({ error: 'Apenas imagens sï¿½o permitidas para thumbnail' });
    }
...
// Upload de vï¿½deo para aula
app.post('/api/upload/video', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    console.log('[POST /api/upload/video] Upload de vï¿½deo iniciado');
    
    if (!req.file) {
      return res.status(400).json({ error: 'Nenhum arquivo enviado' });
    }

    // Verificar se ï¿½ um vï¿½deo
    if (!req.file.mimetype.startsWith('video/')) {
      return res.status(400).json({ error: 'Apenas vï¿½deos sï¿½o permitidos' });
    }
    ...
    res.json({
      ...
      fileName: result.fileName,
      size: result.size,
      message: 'Vï¿½deo enviado com sucesso!'
    });
  } catch (error) {
    console.error('[POST /api/upload/video] Erro:', error);
    res.status(500).json({ 
      error: 'Erro no upload do vï¿½deo',
      details: error.message 
    });
  }
});
...
app.post('/api/upload/avatar', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    ...
    // Verificar se ï¿½ uma imagem
    if (!req.file.mimetype.startsWith('image/')) {
      return res.status(400).json({ error: 'Apenas imagens sï¿½o permitidas para avatar' });
    }
...
// Endpoint para upload de vï¿½deo de aula
app.post('/api/upload/lesson-video', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'Arquivo nï¿½o enviado.' });
    }
    // Apenas vï¿½deos
    if (!req.file.mimetype.startsWith('video/')) {
      return res.status(400).json({ error: 'Apenas arquivos de vï¿½deo sï¿½o permitidos.' });
    }
    const result = await uploadFile(req.file, 'lessons');
    res.json({ url: result.url });
  } catch (err) {
    console.error('[POST /api/upload/lesson-video] Erro:', err);
    res.status(500).json({ error: 'Erro ao fazer upload do vï¿½deo.' });
  }
});

// Endpoint para obter URL assinada do vï¿½deo da aula
app.get('/api/lessons/:lessonId/video-url', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length || !rows[0].video_url) {
      return res.status(404).json({ error: 'Vï¿½deo nï¿½o encontrado para esta aula.' });
    }
...
// Endpoint para marcar aula como concluï¿½da
app.post('/api/lessons/:lessonId/complete', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao marcar aula como concluï¿½da.' });
  }
});

// ===== SISTEMA DE COMENTï¿½RIOS EM AULAS =====

// Buscar comentï¿½rios de uma aula
...
app.post('/api/lessons/:lessonId/comments', authenticateToken, async (req, res) => {
  try {
    ...
    if (!content || content.trim().length === 0) {
      return res.status(400).json({ error: 'Conteï¿½do do comentï¿½rio ï¿½ obrigatï¿½rio.' });
    }
    ...
    // Criar notificaï¿½ï¿½es
    try {
      if (parent_id) {
        // ï¿½ uma resposta a um comentï¿½rio - notificar autor do comentï¿½rio pai
        ...
        if (parentCommentResult.rows.length > 0) {
          ...
          if (parentAuthorId !== userId) {
            ...
            await createNotification(
              parentAuthorId,
              'Resposta ao seu comentï¿½rio',
              `${userName} respondeu ao seu comentï¿½rio na aula "${lesson_title}"`,
              'reply',
              lessonId,
              'lesson'
            );
        ...
        // ï¿½ um comentï¿½rio novo - notificar instrutor da aula
      ...
      console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de comentï¿½rio:', notificationErr);
    }

    res.json({ success: true, comment_id: rows[0].id });
  } catch (err) {
    console.error('[POST /api/lessons/:lessonId/comments]', err);
    res.status(500).json({ error: 'Erro ao criar comentï¿½rio.' });
  }
});

// Atualizar comentï¿½rio
app.put('/api/comments/:commentId', authenticateToken, async (req, res) => {
  try {
    ...
    // Verificar se o comentï¿½rio existe e pertence ao usuï¿½rio
    ...
    if (commentCheck.rows.length === 0) {
      return res.status(404).json({ error: 'Comentï¿½rio nï¿½o encontrado ou nï¿½o autorizado.' });
    }
    ...
    res.status(500).json({ error: 'Erro ao atualizar comentï¿½rio.' });
  }
});
...
app.delete('/api/comments/:commentId', authenticateToken, async (req, res) => {
  try {
    ...
    // Deletar comentï¿½rio (cascade irï¿½ deletar respostas e curtidas)
...
app.post('/api/comments/:commentId/like', authenticateToken, async (req, res) => {
  try {
    ...
    if (likeCheck.rows.length > 0) {
      ...
      // Criar notificaï¿½ï¿½o para o autor do comentï¿½rio (se nï¿½o for o mesmo usuï¿½rio)
      try {
        ...
        if (commentAuthorResult.rows.length > 0) {
          ...
          if (commentAuthorId !== userId) {
            ...
            await createNotification(
              commentAuthorId,
              'Curtida no seu comentï¿½rio',
              `${userName} curtiu seu comentï¿½rio "${shortContent}" na aula "${lesson_title}"`,
              'like',
              commentId,
              'lesson_comment'
            );
        ...
        console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de curtida em comentï¿½rio:', notificationErr);
      }
    }
    ...
    res.status(500).json({ error: 'Erro ao curtir comentï¿½rio.' });
  }
});

// ===== SISTEMA DE NOTIFICAï¿½ï¿½ES =====

// Funï¿½ï¿½o helper para criar notificaï¿½ï¿½es
async function createNotification(userId, title, message, type, referenceId = null, referenceType = null) {
  try {
    console.log('[CREATE NOTIFICATION] Tentando criar notificaï¿½ï¿½o:', {
    ...
    console.log('[CREATE NOTIFICATION] Notificaï¿½ï¿½o criada com sucesso! ID:', result.rows[0].id);
  } catch (err) {
    console.error('[CREATE NOTIFICATION] ERRO ao criar notificaï¿½ï¿½o:', err);
    console.error('[CREATE NOTIFICATION] Parï¿½metros:', {
      userId, title, message, type, referenceId, referenceType
    });
  }
}

// Funï¿½ï¿½o helper para obter nome do usuï¿½rio
async function getUserName(userId, currentUserName = null) {
  // Se jï¿½ temos o nome, usar ele
  if (currentUserName) {
    return currentUserName;
  }
  
  // Caso contrï¿½rio, buscar no banco
  try {
    const { rows } = await pool.query('SELECT name FROM profiles WHERE id = $1', [userId]);
    return rows.length > 0 ? rows[0].name : 'Usuï¿½rio';
  } catch (err) {
...
app.get('/api/notifications', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar notificaï¿½ï¿½es.' });
  }
});
...
app.get('/api/notifications/count', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar contador de notificaï¿½ï¿½es.' });
  }
});
...
app.put('/api/notifications/:notificationId/read', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao marcar notificaï¿½ï¿½o como lida.' });
  }
});

// Marcar todas as notificaï¿½ï¿½es como lidas
app.put('/api/notifications/read-all', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao marcar todas as notificaï¿½ï¿½es como lidas.' });
  }
});

// Deletar notificaï¿½ï¿½o especï¿½fica
app.delete('/api/notifications/:notificationId', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Notificaï¿½ï¿½o nï¿½o encontrada.' });
    }
    
    res.json({ success: true });
  } catch (err) {
    console.error('[DELETE /api/notifications/:id]', err);
    res.status(500).json({ error: 'Erro ao deletar notificaï¿½ï¿½o.' });
  }
});

// Deletar mï¿½ltiplas notificaï¿½ï¿½es
app.delete('/api/notifications', authenticateToken, async (req, res) => {
  try {
    ...
    if (!Array.isArray(notificationIds) || notificationIds.length === 0) {
      return res.status(400).json({ error: 'IDs das notificaï¿½ï¿½es sï¿½o obrigatï¿½rios.' });
    }
    ...
    res.status(500).json({ error: 'Erro ao deletar notificaï¿½ï¿½es.' });
  }
});

// Deletar todas as notificaï¿½ï¿½es
app.delete('/api/notifications/all', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao deletar todas as notificaï¿½ï¿½es.' });
  }
});

// Navegaï¿½ï¿½o inteligente
  ...
  try {
    ...
    switch (reference_type) {
      ...
      case 'forum_topic':
        ...
        break;
      case 'forum_reply':
        // Para respostas do fï¿½rum, navegar para o post pai
      ...
      case 'lesson':
        ...
        break;
      case 'lesson_comment':
        // Para comentï¿½rios de aula, navegar para a aula
    ...
    res.status(500).json({ error: 'Erro ao navegar pela notificaï¿½ï¿½o.' });
  }
});


// ===== SISTEMA DE AVALIAï¿½ï¿½ES DE CURSOS =====

// Buscar avaliaï¿½ï¿½es de um curso
app.get('/api/courses/:courseId/ratings', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar avaliaï¿½ï¿½es.' });
  }
});

// Buscar estatï¿½sticas de avaliaï¿½ï¿½es de um curso
app.get('/api/courses/:courseId/rating-stats', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar estatï¿½sticas de avaliaï¿½ï¿½o.' });
  }
});

// Buscar avaliaï¿½ï¿½o do usuï¿½rio atual para um curso
app.get('/api/courses/:courseId/my-rating', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar avaliaï¿½ï¿½o do usuï¿½rio.' });
  }
});

// Criar ou atualizar avaliaï¿½ï¿½o de um curso
app.post('/api/courses/:courseId/rate', authenticateToken, async (req, res) => {
  try {
    ...
    if (!rating || rating < 1 || rating > 5) {
      return res.status(400).json({ error: 'Avaliaï¿½ï¿½o deve ser entre 1 e 5 estrelas.' });
    }
    ...
    // Verificar se jï¿½ existe uma avaliaï¿½ï¿½o do usuï¿½rio
    ...
    if (existingRating.rows.length > 0) {
      // Atualizar avaliaï¿½ï¿½o existente
      await pool.query(
...
// Deletar avaliaï¿½ï¿½o do usuï¿½rio
app.delete('/api/courses/:courseId/rate', authenticateToken, async (req, res) => {
  try {
    ...
    if (rowCount === 0) {
      return res.status(404).json({ error: 'Avaliaï¿½ï¿½o nï¿½o encontrada.' });
    }

    res.json({ success: true });
  } catch (err) {
    console.error('[DELETE /api/courses/:courseId/rate]', err);
    res.status(500).json({ error: 'Erro ao deletar avaliaï¿½ï¿½o.' });
  }
});

// ===== SISTEMA DE MENSAGENS =====

// Buscar conversas do usuï¿½rio
...
// Buscar conversa especï¿½fica com mensagens
app.get('/api/conversations/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // Verificar se o usuï¿½rio ï¿½ participante da conversa
    ...
    if (conversationResult.rows.length === 0) {
      return res.status(404).json({ error: 'Conversa nï¿½o encontrada.' });
    }
    ...
    res.json({
      ...
      messages: messagesResult.rows.reverse(), // Ordem cronolï¿½gica
...
// Criar conversa direta entre dois usuï¿½rios
app.post('/api/conversations/direct', authenticateToken, async (req, res) => {
  try {
    ...
    if (!otherUserId) {
      return res.status(400).json({ error: 'ID do outro usuï¿½rio ï¿½ obrigatï¿½rio.' });
    }

    if (otherUserId === userId) {
      return res.status(400).json({ error: 'Nï¿½o ï¿½ possï¿½vel criar conversa consigo mesmo.' });
    }

    // Verificar se o outro usuï¿½rio existe
    const userCheck = await pool.query('SELECT 1 FROM profiles WHERE id = $1', [otherUserId]);
    if (userCheck.rows.length === 0) {
      return res.status(404).json({ error: 'Usuï¿½rio nï¿½o encontrado.' });
    }

    // Usar funï¿½ï¿½o do banco para obter ou criar conversa
    const { rows } = await pool.query(
...
app.post('/api/conversations/:id/messages', authenticateToken, async (req, res) => {
  try {
    ...
    if (!content || content.trim() === '') {
      return res.status(400).json({ error: 'Conteï¿½do da mensagem ï¿½ obrigatï¿½rio.' });
    }
    ...
    if (reply_to_id) {
      ...
      if (replyCheck.rows.length === 0) {
        return res.status(400).json({ error: 'Mensagem de resposta nï¿½o encontrada.' });
      }
    }
    ...
    // Criar notificaï¿½ï¿½es para outros participantes
...
app.get('/api/conversations/:id/messages', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      messages: rows.reverse(), // Ordem cronolï¿½gica
...
// Buscar usuï¿½rios para iniciar conversa
app.get('/api/users/search', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao buscar usuï¿½rios.' });
  }
});

// Contar mensagens nï¿½o lidas
app.get('/api/conversations/unread-count', authenticateToken, async (req, res) => {
  try {
    ...
    // Se as tabelas nï¿½o existem, retornar 0
    if (!conversations_exist || !participants_exist || !messages_exist) {
      return res.json({ 
        unread_count: 0,
        total_count: 0,
        message: 'Sistema de mensagens nï¿½o configurado'
      });
    }
    ...
    res.json({ 
      unread_count: 0,
      total_count: 0,
      error: 'Erro ao contar mensagens nï¿½o lidas.'
    });
  }
});
...
// Endpoint para buscar estatï¿½sticas de um curso
app.get('/api/courses/:courseId/stats', authenticateToken, async (req, res) => {
  try {
    const { courseId } = req.params;
    
    console.log('[GET /api/courses/:courseId/stats] Buscando estatï¿½sticas do curso:', courseId);
    ...
    console.error('[GET /api/courses/:courseId/stats] Erro ao buscar estatï¿½sticas:', err);
    res.status(500).json({ error: 'Erro interno.' });
  }
});
...
app.get('/api/debug/tables', authenticateToken, async (req, res) => {
  try {
    ...
    // Verificar se hï¿½ dados nas tabelas
...
app.get('/api/classes/:id/courses', authenticateToken, async (req, res) => {
  try {
    ...
    const { id } = req.params;
    process.stdout.write('\n[GET /api/classes/:id/courses] === INï¿½CIO DA REQUISIï¿½ï¿½O ===\n');
    process.stdout.write(`[GET /api/classes/:id/courses] ID da turma: ${id}\n`);
    process.stdout.write(`[GET /api/classes/:id/courses] Usuï¿½rio: ${JSON.stringify(req.user)}\n`);
    
    // Primeiro, buscar a turma para verificar se o usuï¿½rio tem acesso
    const classResult = await pool.query(`
      SELECT instructor_id, is_public FROM class_instances WHERE id = $1
    `, [id]);
    ...
    if (classResult.rows.length === 0) {
      process.stdout.write('[GET /api/classes/:id/courses] Turma nï¿½o encontrada\n');
      return res.status(404).json({ error: 'Turma nï¿½o encontrada.' });
    }
    ...
    // Verificar se o usuï¿½rio tem acesso aos cursos
    // 1. Se ï¿½ o instructor da turma
    if (classData.instructor_id === req.user.id) {
      process.stdout.write('[GET /api/classes/:id/courses] Usuï¿½rio ï¿½ o instructor da turma\n');
      ...
      process.stdout.write('[GET /api/classes/:id/courses] === FIM DA REQUISIï¿½ï¿½O (instructor) ===\n\n');
      
      return res.json(rows);
    }
    
    // 2. Se a turma ï¿½ pï¿½blica, permitir visualizar cursos
    if (classData.is_public) {
      process.stdout.write('[GET /api/classes/:id/courses] Turma ï¿½ pï¿½blica, acesso permitido\n');
      process.stdout.write('[GET /api/classes/:id/courses] Executando query para buscar cursos...\n');
      ...
      process.stdout.write(`[GET /api/classes/:id/courses] Cursos encontrados: ${rows.length}\n`);
      process.stdout.write(`[GET /api/classes/:id/courses] Dados dos cursos: ${JSON.stringify(rows)}\n`);
      process.stdout.write('[GET /api/classes/:id/courses] === FIM DA REQUISIï¿½ï¿½O (pï¿½blica) ===\n\n');
    ...
    process.stdout.write(`[GET /api/classes/:id/courses] Resultado da verificaï¿½ï¿½o de matrï¿½cula: ${JSON.stringify(enrollmentCheck.rows)}\n`);
    
    if (enrollmentCheck.rows.length > 0) {
      process.stdout.write('[GET /api/classes/:id/courses] Usuï¿½rio estï¿½ matriculado na turma\n');
      process.stdout.write('[GET /api/classes/:id/courses] Executando query para buscar cursos...\n');
      ...
      process.stdout.write(`[GET /api/classes/:id/courses] Cursos encontrados: ${rows.length}\n`);
      process.stdout.write(`[GET /api/classes/:id/courses] Dados dos cursos: ${JSON.stringify(rows)}\n`);
      process.stdout.write('[GET /api/classes/:id/courses] === FIM DA REQUISIï¿½ï¿½O (matriculado) ===\n\n');
      
      return res.json(rows);
    }
    
    process.stdout.write('[GET /api/classes/:id/courses] Acesso negado - usuï¿½rio nï¿½o tem permissï¿½o\n');
    process.stdout.write('[GET /api/classes/:id/courses] === FIM DA REQUISIï¿½ï¿½O (acesso negado) ===\n\n');
    return res.status(403).json({ error: 'Acesso negado a esta turma.' });
...
// Adicionar curso ï¿½ turma
app.post('/api/classes/:id/courses', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { course_id, is_required = false, order_index } = req.body;
    
    console.log('[POST /api/classes/:id/courses] Adicionando curso ï¿½ turma:', id);
    console.log('[POST /api/classes/:id/courses] Dados recebidos:', req.body);
    console.log('[POST /api/classes/:id/courses] Usuï¿½rio:', req.user);
    
    if (!course_id) {
      console.log('[POST /api/classes/:id/courses] Erro: course_id nï¿½o fornecido');
      return res.status(400).json({ error: 'course_id ï¿½ obrigatï¿½rio.' });
    }
    
    // Verificar se o usuï¿½rio ï¿½ instructor da turma ou admin
    const classCheck = await pool.query(`
      SELECT instructor_id FROM class_instances WHERE id = $1
    `, [id]);
    
    if (classCheck.rows.length === 0) {
      console.log('[POST /api/classes/:id/courses] Erro: Turma nï¿½o encontrada');
      return res.status(404).json({ error: 'Turma nï¿½o encontrada.' });
    }
    
    console.log('[POST /api/classes/:id/courses] Instructor da turma:', classCheck.rows[0].instructor_id);
    console.log('[POST /api/classes/:id/courses] Usuï¿½rio atual:', req.user.id);
    console.log('[POST /api/classes/:id/courses] Role do usuï¿½rio:', req.user.role);
    ...
    const courseCheck = await pool.query('SELECT id FROM courses WHERE id = $1', [course_id]);
    if (courseCheck.rows.length === 0) {
      console.log('[POST /api/classes/:id/courses] Erro: Curso nï¿½o encontrado');
      return res.status(404).json({ error: 'Curso nï¿½o encontrado.' });
    }
    
    // Verificar se o curso jï¿½ estï¿½ na turma
    const existingCheck = await pool.query(`
      SELECT id FROM class_courses WHERE class_id = $1 AND course_id = $2
    `, [id, course_id]);
    
    if (existingCheck.rows.length > 0) {
      console.log('[POST /api/classes/:id/courses] Erro: Curso jï¿½ estï¿½ na turma');
      return res.status(400).json({ error: 'Este curso jï¿½ estï¿½ associado ï¿½ turma.' });
    }
...
// Atualizar configuraï¿½ï¿½es de um curso na turma
app.put('/api/classes/:id/courses/:courseId', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[PUT /api/classes/:id/courses/:courseId] Usuï¿½rio:', req.user);
    ...
    // Verificar se o curso estï¿½ na turma
    const courseCheck = await pool.query(`
      SELECT id FROM class_courses WHERE class_id = $1 AND course_id = $2
    `, [id, courseId]);
    
    if (courseCheck.rows.length === 0) {
      return res.status(404).json({ error: 'Curso nï¿½o encontrado na turma.' });
    }
...
app.delete('/api/classes/:id/courses/:courseId', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[DELETE /api/classes/:id/courses/:courseId] Usuï¿½rio:', req.user);
    ...
    // Nï¿½o permitir remover o curso obrigatï¿½rio (primeiro curso)
    if (courseCheck.rows[0].is_required) {
      return res.status(400).json({ error: 'Nï¿½o ï¿½ possï¿½vel remover o curso obrigatï¿½rio da turma.' });
    }
...
app.get('/api/test/class-courses', async (req, res) => {
  try {
    ...
    // Verificar se a funï¿½ï¿½o existe
...
app.get('/api/test/class-courses-view', authenticateToken, async (req, res) => {
  try {
    ...
    if (viewCheck.rows.length === 0) {
      return res.status(500).json({ error: 'View class_courses_with_details nï¿½o existe' });
    }
    ...
    res.json({
      message: 'View class_courses_with_details estï¿½ funcionando',
...
app.get('/api/test/class-courses-table', authenticateToken, async (req, res) => {
  try {
    ...
    if (tableCheck.rows.length === 0) {
      return res.status(500).json({ error: 'Tabela class_courses nï¿½o existe' });
    }
    ...
    res.json({
      message: 'Tabela class_courses estï¿½ funcionando',
      table_exists: true,
...
app.get('/api/test/class-courses-data', authenticateToken, async (req, res) => {
  try {
    ...
    // Verificar se hï¿½ dados na tabela
    const countResult = await pool.query('SELECT COUNT(*) as total FROM class_courses');
...
// Criar novo tï¿½pico (apenas admin/instructor)
app.post('/api/forum/topics', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[POST /api/forum/topics] Criando tï¿½pico:', title);
    console.log('[POST /api/forum/topics] Usuï¿½rio:', req.user);
    
    // Verificar permissï¿½o
    if (!['admin', 'instructor'].includes(req.user.role)) {
      console.log('[POST /api/forum/topics] Acesso negado para role:', req.user.role);
      return res.status(403).json({ error: 'Apenas administradores e instrutores podem criar tï¿½picos.' });
    }
    
    if (!title || title.trim().length === 0) {
      return res.status(400).json({ error: 'Tï¿½tulo ï¿½ obrigatï¿½rio.' });
    }
    ...
    console.log('[POST /api/forum/topics] Tï¿½pico criado com sucesso:', rows[0].title);
    res.status(201).json(rows[0]);
  } catch (err) {
...
// Editar tï¿½pico (apenas admin/instructor ou criador)
app.put('/api/forum/topics/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[PUT /api/forum/topics/:id] Editando tï¿½pico:', id);
    console.log('[PUT /api/forum/topics/:id] Usuï¿½rio:', req.user);
    
    if (!title || title.trim().length === 0) {
      return res.status(400).json({ error: 'Tï¿½tulo ï¿½ obrigatï¿½rio.' });
    }
    
    // Verificar se o tï¿½pico existe
    const topicCheck = await pool.query(`
      SELECT * FROM forum_topics WHERE id = $1
    `, [id]);
    
    if (topicCheck.rows.length === 0) {
      return res.status(404).json({ error: 'Tï¿½pico nï¿½o encontrado.' });
    }
    
    const topic = topicCheck.rows[0];
    
    // Verificar permissï¿½o (admin, instructor ou criador do tï¿½pico)
    if (!['admin', 'instructor'].includes(req.user.role) && topic.created_by !== req.user.id) {
      return res.status(403).json({ error: 'Vocï¿½ nï¿½o tem permissï¿½o para editar este tï¿½pico.' });
    }
    
    // Gerar novo slug se o tï¿½tulo mudou
    let slug = topic.slug;
    if (title.trim() !== topic.title) {
      ...
      // Verificar se slug jï¿½ existe (excluindo o tï¿½pico atual)
      while (true) {
    ...
    console.log('[PUT /api/forum/topics/:id] Tï¿½pico editado com sucesso:', rows[0].title);
    res.json(rows[0]);
  } catch (err) {
...
// Excluir tï¿½pico (apenas admin/instructor ou criador)
app.delete('/api/forum/topics/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    
    console.log('[DELETE /api/forum/topics/:id] Excluindo tï¿½pico:', id);
    console.log('[DELETE /api/forum/topics/:id] Usuï¿½rio:', req.user);
    ...
    // Verificar permissï¿½o (admin, instructor ou criador do tï¿½pico)
    if (!['admin', 'instructor'].includes(req.user.role) && topic.created_by !== req.user.id) {
      return res.status(403).json({ error: 'Vocï¿½ nï¿½o tem permissï¿½o para excluir este tï¿½pico.' });
    }
    
    // Verificar se hï¿½ posts no tï¿½pico
    ...
    if (postsCount > 0) {
      return res.status(400).json({ 
        error: `Nï¿½o ï¿½ possï¿½vel excluir o tï¿½pico. Ele possui ${postsCount} post(s). Remova todos os posts primeiro.` 
      });
    }
    ...
    console.log('[DELETE /api/forum/topics/:id] Tï¿½pico excluï¿½do com sucesso:', topic.title);
    res.json({ message: 'Tï¿½pico excluï¿½do com sucesso.' });
  } catch (err) {
...
// Listar posts de um tï¿½pico
app.get('/api/forum/topics/:slug/posts', authenticateToken, async (req, res) => {
  try {
    const { slug } = req.params;
    const { page = 1, limit = 20, sort = 'latest' } = req.query;
    
    console.log('[GET /api/forum/topics/:slug/posts] Buscando posts do tï¿½pico:', slug);
    ...
    // Construir query de ordenaï¿½ï¿½o
...
app.post('/api/forum/posts', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[POST /api/forum/posts] Usuï¿½rio:', req.user);
    
    if (!topic_id || !title || !content) {
      return res.status(400).json({ error: 'Tï¿½pico, tï¿½tulo e conteï¿½do sï¿½o obrigatï¿½rios.' });
    }
    
    // Verificar se o tï¿½pico existe e estï¿½ ativo
    const topicCheck = await pool.query(`
      SELECT id FROM forum_topics WHERE id = $1 AND is_active = true
    `, [topic_id]);
    
    if (topicCheck.rows.length === 0) {
      return res.status(404).json({ error: 'Tï¿½pico nï¿½o encontrado ou inativo.' });
    }
    ...
    try {
      ...
      if (tags.length > 0) {
        for (const tagName of tags) {
          // Verificar se tag existe, se nï¿½o, criar
      ...
      // Criar notificaï¿½ï¿½o para moderadores/admins sobre novo post
      try {
        const modAdminResult = await pool.query(`
          SELECT id, name
          FROM profiles
        ...
        if (topicResult.rows.length > 0) {
          ...
          for (const moderator of modAdminResult.rows) {
            await createNotification(
              moderator.id,
              'Novo post no fï¿½rum',
              `${userName} criou um novo post "${title}" no tï¿½pico "${topicTitle}"`,
              'forum_new_post',
        ...
        console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de novo post:', notificationErr);
      }
...
// Buscar post especï¿½fico com respostas
app.get('/api/forum/posts/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // Incrementar contador de visualizaï¿½ï¿½es
    await pool.query(`
...
app.post('/api/forum/posts/:id/replies', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[POST /api/forum/posts/:id/replies] Usuï¿½rio:', req.user);
    
    if (!content || content.trim().length === 0) {
      return res.status(400).json({ error: 'Conteï¿½do ï¿½ obrigatï¿½rio.' });
    }
    
    // Verificar se o post existe e nï¿½o estï¿½ bloqueado
    ...
    if (postCheck.rows[0].is_locked) {
      return res.status(400).json({ error: 'Este post estï¿½ bloqueado para novas respostas.' });
    }
    ...
    // Criar notificaï¿½ï¿½o para o autor do post (se nï¿½o for o mesmo usuï¿½rio)
    try {
      const postAuthorResult = await pool.query(`
      ...
      console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de resposta:', notificationErr);
    }
  ...
  try {
    ...
    if (likeCheck.rows.length > 0) {
      ...
      try {
        ...
        console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de curtida:', notificationErr);
      }
    }
...
app.post('/api/forum/replies/:id/like', authenticateToken, async (req, res) => {
  try {
    ...
    if (likeCheck.rows.length > 0) {
      ...
      // Criar notificaï¿½ï¿½o para o autor da resposta (se nï¿½o for o mesmo usuï¿½rio)
      try {
        ...
        console.error('[NOTIFICATION] Erro ao criar notificaï¿½ï¿½o de curtida em resposta:', notificationErr);
      }
    }
...
// Editar comentï¿½rio/resposta do fï¿½rum
app.put('/api/forum/replies/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[PUT /api/forum/replies/:id] Usuï¿½rio:', req.user);
    
    if (!content || content.trim().length === 0) {
      return res.status(400).json({ error: 'Conteï¿½do da resposta ï¿½ obrigatï¿½rio.' });
    }
    
    // Verificar se a resposta existe e se o usuï¿½rio tem permissï¿½o para editar
    const replyCheck = await pool.query(`
      SELECT author_id FROM forum_replies WHERE id = $1
    `, [replyId]);
    
    if (replyCheck.rows.length === 0) {
      return res.status(404).json({ error: 'Resposta nï¿½o encontrada.' });
    }
    
    const reply = replyCheck.rows[0];
    
    // Verificar permissï¿½es (apenas autor ou admin podem editar)
    if (reply.author_id !== req.user.id && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Vocï¿½ nï¿½o tem permissï¿½o para editar esta resposta.' });
    }
...
// Deletar comentï¿½rio/resposta do fï¿½rum
app.delete('/api/forum/replies/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[DELETE /api/forum/replies/:id] Usuï¿½rio:', req.user);
    
    // Verificar se a resposta existe e se o usuï¿½rio tem permissï¿½o para deletar
    const replyCheck = await pool.query(`
      SELECT author_id FROM forum_replies WHERE id = $1
    `, [replyId]);
    ...
    // Verificar permissï¿½es (apenas autor ou admin podem deletar)
    if (reply.author_id !== req.user.id && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Vocï¿½ nï¿½o tem permissï¿½o para deletar esta resposta.' });
    }
...
// ===== ENDPOINTS DE ADMINISTRAï¿½ï¿½O PARA MIGRATION =====
...
// Aplicar migration dos posts do fï¿½rum
app.post('/api/admin/apply-forum-posts-migration', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[MIGRATION] Aplicando migration dos posts do fï¿½rum...');
    ...
    try {
      ...
      if (checkContentImageUrl.rows.length === 0) {
        ...
        console.log('[MIGRATION] Coluna content_image_url jï¿½ existe');
      }
      ...
      if (checkCoverImageUrl.rows.length > 0) {
        ...
        console.log('[MIGRATION] Coluna cover_image_url nï¿½o existe');
      }
...
app.put('/api/forum/posts/:id', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[PUT /api/forum/posts/:id] Usuï¿½rio:', req.user);
    
    if (!title || !content) {
      return res.status(400).json({ error: 'Tï¿½tulo e conteï¿½do sï¿½o obrigatï¿½rios.' });
    }
    
    // Verificar se o post existe e se o usuï¿½rio tem permissï¿½o para editar
    const postCheck = await pool.query(`
    ...
    if (post.author_id !== req.user.id && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Vocï¿½ nï¿½o tem permissï¿½o para editar este post.' });
    }
...
// Excluir post do fï¿½rum
app.delete('/api/forum/posts/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    
    console.log('[DELETE /api/forum/posts/:id] Excluindo post:', id);
    console.log('[DELETE /api/forum/posts/:id] Usuï¿½rio:', req.user);
    
    // Verificar se o post existe e se o usuï¿½rio tem permissï¿½o para excluir
    const postCheck = await pool.query(`
      SELECT author_id FROM forum_posts WHERE id = $1
    `, [id]);
    ...
    // Verificar permissï¿½es (apenas autor ou admin podem excluir)
    if (post.author_id !== req.user.id && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Vocï¿½ nï¿½o tem permissï¿½o para excluir este post.' });
    }
    ...
    try {
      ...
      console.log('[DELETE /api/forum/posts/:id] Post excluï¿½do com sucesso');
      res.json({ success: true, message: 'Post excluï¿½do com sucesso.' });
...
// Buscar respostas de um post especï¿½fico
...
app.get('/api/webhooks', authenticateToken, async (req, res) => {
  try {
    console.log('[GET /api/webhooks] Usuï¿½rio:', req.user);
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Acesso negado.' });
    }
...
app.post('/api/webhooks', authenticateToken, async (req, res) => {
  try {
    ...
    if (!name || !url || !events || !Array.isArray(events)) {
      return res.status(400).json({ error: 'Nome, URL e eventos sï¿½o obrigatï¿½rios.' });
    }
...
app.put('/api/webhooks/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Webhook nï¿½o encontrado.' });
    }
